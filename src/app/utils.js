export function STORAGE_KEY(x) {
  return "Wang2Meet_" + x;
}
export function inRange(v, l, r) {
  return (l <= v && v <= r) || (r <= v && v <= l);
}
export function pad(n, digit) {
  const res = `${n}`;
  return "0".repeat(Math.max(0, digit - res.length)) + res;
}
export class Time {
  constructor(i) {
...
export const defaultTime = [0, 96],
  defaultDate = [2003, 6, 1],
  defaultDuration = 7;
export function dump(table) {
  return table.map((r) => r.map((e) => (e ? "1" : "0")).join("")).join("\n");
}
export function parse(str) {
  return str.split("\n").map((r) => r.split("").map((e) => e === "1"));
}
export function colorScale(c, r) {
  return (
    "#" +
    [c.slice(1, 3), c.slice(3, 5), c.slice(5, 7)]
      .map((e) =>
        (
          "0" +
          parseInt(parseInt(e, 16) * r + 255 * (1 - r))
            .toString(16)
            .toUpperCase()
        ).slice(-2),
      )
...
export function tableMap(t, f) {
  return t.map((row, i) => row.map((e, j) => f(e, i, j)));
}
export function mkTable(r, c, f = null) {
  return new Array(r).fill(0).map(() => new Array(c).fill(f));
}
...
export function cast(table, d1, t1, d2, t2, duration) {
  if (
    ...
    table = table.map((row) =>
      new Array(duration).fill(day0).map((e, i) => row[(e + i) % 7]),
    );
    return table;
  }
}
export function slotBefore(a, b) {
...
export function getTimezoneHere() {
  return Intl.DateTimeFormat().resolvedOptions().timeZone;
}
class TimeAdapter {
  constructor(date, dateDuration, timeDuration, timezone) {
    const localTimeStr = new Date(date).toLocaleString("en-US", {
      hour12: false,
      timeZone: timezone,
    });
    ...
    this.mask = !this.wrap
      ? null
      : tableMap(mkTable(96, this.getDuration()), (_, i, j) => {
  ...
  getTime() {
    return (this.wrap ? [0, 24] : [this.h, this.h + this.tD]).map((e) => e * 4);
  }
  getDuration() {
    return this.wrap ? this.dD + 1 : this.dD;
  }
  idx_tablize(i, j) {
    return !this.wrap
      ? [i, j]
  ...
  tbl_tablize(tbl) {
    return !this.wrap
      ? tbl
      : tableMap(
          mkTable(this.tD * 4, this.dD),
          (_, i, j) =>
            tbl[this.idx_localize(i, j)[0]][this.idx_localize(i, j)[1]],
        );
  }
  getIndex() {
    return this.tbl_localize(
      tableMap(mkTable(this.tD * 4, this.dD), (_, i, j) => i + j * this.tD * 4),
    );
  }
}
export function wrapConfig(cfg, tz) {
  ...
  for (let idx in cfg.collection) {
    ...
    cfg.collection[idx].table = TA.tbl_localize(
      parse(cfg.collection[idx].table),
    );
  }
  // console.log(cfg)
  return cfg;
