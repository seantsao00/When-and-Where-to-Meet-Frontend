...
const Corner = styled(TableCtrl)`
  ...
  background: linear-gradient(
    white,
    white calc(var(--h) - 7px),
    transparent calc(var(--h) - 7px),
    transparent
  );
  height: var(--h);
  width: var(--w);
`;
const ScrollX = styled(TableCtrl)(
  ({ theme }) => `
  top: 0;
  height: var(--h);
  display: flex;
...
const ScrollY = styled(TableCtrl)(
  ({ theme }) => `
  left: 0;
  width: var(--w);
  display: flex;
...
export default function TimeTable({
  ...
  useEffect(() => {
    setRandomID(parseInt(Math.random() * 1e8));
  }, []);

  const ROW = useMemo(
    () => new Array(time[1] - time[0]).fill(time[0]).map((e, i) => e + i),
    [time],
  );
  const dates = useMemo(
    () =>
      new Array(duration)
        .fill(0)
        .map((e, i) => new Date(date[0], date[1] - 1, date[2] + i)),
    [date, duration],
  );
  const COL = useMemo(
    () =>
      new Array(duration)
        .fill(new Date(date[0], date[1] - 1, date[2]).getDay())
        .map((e, i) => e + i - 1),
    [date, duration],
  );
      ...
      >
        ...
        >
          <tbody>
            ...
            <tr>
              <TimeTd>
                {new Time(ROW[ROW.length - 1] + 1).section === 0 &&
                  new Time(ROW[ROW.length - 1] + 1).timeStr}
              </TimeTd>
            </tr>
          </tbody>
        </Table>
