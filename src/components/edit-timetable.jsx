...
export default function EditTimeTable({
  ...
  const EMPTY_TABLE = useMemo(
    () => mkTable(time[1] - time[0], duration, false),
    [time, duration],
  );
  useEffect(() => {
    if (defaultTable) setValue(defaultTable, false);
    else setValue(EMPTY_TABLE, false);
  ...
  const covered = useCallback(
    (i, j) =>
      sel &&
      inRange(i, sel[0][0], sel[1][0]) &&
      inRange(j, sel[0][1], sel[1][1]),
    [sel],
  );
  const newTable = useCallback(
    (i, j) => (covered(i, j) ? !value[sel[0][0]][sel[0][1]] : value[i][j]),
    [sel, value, covered],
  );
  const up = useCallback(() => {
    if (sel) {
      const res = tableMap(
        EMPTY_TABLE,
        (e, i, j) => newTable(i, j) && !(mask && mask[i][j]),
      );
      setValue(res);
      if (bufferTime) {
